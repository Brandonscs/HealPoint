...
export default function CitaMedicoList() {
  ...
  const cargarCitas = useCallback(async (idMedico) => {
    ...
    try {
      ...
      // El backend puede devolver un string si no hay citas o un array si hay citas
      if (typeof response.data === 'string') {
        // El backend devolvió un mensaje de "no hay citas"
        console.log("No hay citas para este médico (mensaje del servidor):", response.data);
        setCitas([]);
        setCitasFiltradas([]);
      } else if (Array.isArray(response.data)) {
        // El backend devolvió un array de citas
  ...
  const cargarMedicoLogueado = useCallback(async () => {
    try {
      ...
      // IMPORTANTE: Verificar el nombre correcto de la propiedad ID del médico
      ...
      // Cargar citas después de obtener el médico
      if (idMedicoReal) {
  ...
  const actualizarEstadoCita = async () => {
    ...
    try {
      const payload = {
        ...
        paciente: { idPaciente: citaSeleccionada.paciente.idPaciente },
        medico: { id_medico: citaSeleccionada.medico.id_medico },
      ...
      await citaService.actualizarCita(citaSeleccionada.id_cita, payload);
      setSuccess("Estado de la cita actualizado exitosamente");
      await cargarCitas(medicoLogueado.idMedico);
      cerrarModalEstado();
            ...
            citasFiltradas.map((cita) => (
              <div key={cita.id_cita} className="cita-item">
                <div className="cita-header">
                  ...
                  <span className={`estado-badge ${obtenerColorEstado(cita.id_estado)}`}>
                    {obtenerTextoEstado(cita.id_estado)}
                  </span>
                </div>
              ...
              <div className="detalle-section">
                ...
                <div className="detalle-grid">
                  ...
                  <div className="detalle-item">
                    ...
                    <span className={`estado-badge ${obtenerColorEstado(citaSeleccionada.id_estado)}`}>
                      {obtenerTextoEstado(citaSeleccionada.id_estado)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="detalle-section">
                ...
                <div className="detalle-grid">
                  ...
                  <div className="detalle-item">
                    ...
                    <span className="detalle-value">{citaSeleccionada.paciente?.usuario?.numeroDocumento}</span>
                  </div>
                  <div className="detalle-item">
          ...
          <div className="modal-content modal-estado" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="cita-info-resumen">
                <p><strong>Paciente:</strong> {citaSeleccionada.id_paciente?.usuario?.nombre} {citaSeleccionada.paciente?.usuario?.apellido}</p>
                <p><strong>Fecha:</strong> {formatearFecha(citaSeleccionada.fecha)} - {formatearHora(citaSeleccionada.hora)}</p>
                <p><strong>Estado Actual:</strong> <span className={`estado-badge ${obtenerColorEstado(citaSeleccionada.id_estado)}`}>{obtenerTextoEstado(citaSeleccionada.id_estado)}</span></p>
              </div>
