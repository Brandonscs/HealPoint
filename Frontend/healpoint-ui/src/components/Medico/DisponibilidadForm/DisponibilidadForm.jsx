...
export default function DisponibilidadForm() {
  ...
  const cargarDisponibilidades = useCallback(async (idMedico) => {
    if (!idMedico) {
      console.log("No hay idMedico para cargar disponibilidades");
      return;
    }

    console.log("Cargando disponibilidades para médico ID:", idMedico);
    ...
    try {
      ...
      console.log("Respuesta de disponibilidades:", response);
      ...
      if (Array.isArray(response.data)) {
        ...
        console.log("Disponibilidades cargadas:", ordenadas);
        setDisponibilidades(ordenadas);
      } else if (typeof response.data === 'string') {
        console.log("Sin disponibilidades (mensaje):", response.data);
        setDisponibilidades([]);
      } else {
      ...
      // Si el error es 404 o no hay datos, no mostrar error
      if (err.response?.status === 404 || err.response?.data?.includes("No se encontr")) {
        console.log("No hay disponibilidades registradas aún");
        setDisponibilidades([]);
      } else {
  ...
  const cargarMedicoLogueado = useCallback(async () => {
    try {
      ...
      console.log("Obteniendo médico para usuario ID:", usuarioLocal.idUsuario);
      const response = await axios.get(
        `http://localhost:8080/medico/usuario/${usuarioLocal.idUsuario}`
      );

      console.log("Médico obtenido:", response.data);
      ...
      setFormData(prev => ({ ...prev, idMedico: idMedicoReal }));
  ...
  const abrirModalCrear = () => {
    ...
    setFormData({
      idMedico: medicoLogueado.idMedico || medicoLogueado.id_medico,
      diaSemana: "",
      horaInicio: "",
      horaFin: "",
      estado: "DISPONIBLE"
  ...
  const abrirModalEditar = (disponibilidad) => {
    ...
    setFormData({
      idMedico: disponibilidad.medico?.idMedico || medicoLogueado.idMedico || medicoLogueado.id_medico,
